"""Hackathon demo entrypoint placeholder using the AgentGuard v1 firewall."""
...
def main() -> None:
    trace = TraceV1Builder().build(
        TraceV1BuildInput(
            session_id="google_adk_demo_session",
            step_index=1,
    ...
    result = AgentGuardFirewallV1(trace_store=TraceStore(), namespace="google_adk_demo").intercept(
        trace
    )
    print(f"Generated 1 demo v1 trace. Decision: {result.decision.decision}")
