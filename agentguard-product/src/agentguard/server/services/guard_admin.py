...
def guard_admin_status(
    ...
    mode = os.environ.get("AGENTGUARD_FIREWALL_MODE", "v2").lower()
    ...
    components = [
        GuardAdminComponent(
            ...
            name="Tool interception integration",
            layer="Runtime",
            status="observe_only",
            summary=(
                "The registered agent manifest is available. Remote interception "
                "transport is the next implementation phase."
            ),
            management=(
                "The agent will submit proposals through the public AgentGuard SDK."
            ),
        ),
        GuardAdminComponent(
        ...
        GuardAdminComponent(
            ...
            status="operational",
            summary=(
                "One immutable intent contract is extracted at the start of each user "
                "turn and referenced by every proposed tool call."
            ),
            management=(
                "Gemini structured output is the primary extractor. Schema validation "
                "and a conservative deterministic fallback protect availability. Tier 1 "
                "enforces capabilities, resources, destinations, and side-effect scope."
            ),
        ),
        GuardAdminComponent(
            component_id="evaluation_router",
            name="Policy-driven tier router",
            layer="Evaluation planning",
            status="operational",
            summary=(
                "Each normalized action is classified as read-only, reversible, "
                "external or irreversible, or ambiguous. The active policy selects "
                "the required tiers for that class."
            ),
            management=(
                "Routing is configured in policy.routing. Tier 1 always runs; a "
                "deterministic block short-circuits later tiers, and missing required "
                "tiers use the policy's fail-safe defaults."
            ),
        ),
        GuardAdminComponent(
            component_id="tier_1",
        ...
        GuardAdminComponent(
            ...
            status=(
                ...
                and runtime_config.tier_3_enforcement_enabled
                else "observe_only"
                if runtime_config.tier_3_enabled
                else "disabled"
            ),
            summary=(
                "The Gemini structured-output judge participates in policy-routed "
                "evaluations and can escalate an allow decision."
                if runtime_config.tier_3_enabled
                and runtime_config.tier_3_enforcement_enabled
                else "The Gemini judge runs on policy-routed evaluations, but its "
                "result is recorded without changing execution."
                if runtime_config.tier_3_enabled
            ...
            management=(
                ...
                "non-overridable, model blocks are capped to approval, and judge "
                "failure or low confidence fails to approval."
            ),
        ),
        GuardAdminComponent(
        ...
        GuardAdminComponent(
            ...
            management=(
                "Set AGENTGUARD_FIREWALL_MODE=v2 and restart the AgentGuard API."
            ),
        ),
    ]
    warnings = []
    ...
    warnings.append(
        "Metadata-driven normalization, turn-scoped intent authorization, and AgentTrust "
        "shell security are active. Tier 2 semantic analysis, approval resume, and "
        "declarative descriptor overrides remain incomplete."
    )
    return GuardAdminStatus(
        ...
        tools=[_registered_tool_view(tool) for tool in definition.tools],
        warnings=warnings,
    )
...
def _registered_tool_view(tool) -> AgentToolDefinition:
