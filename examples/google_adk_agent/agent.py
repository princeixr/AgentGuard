...
Run it with the ADK CLI from the repo root:
    ...
    adk run examples/google_adk_agent
    adk web                       # browser UI; pick "adk_agent"

or with the bundled standalone chat loop:

    python examples/google_adk_agent/chat.py
...
from agentguard.integrations.google_adk import GoogleADKTraceSession, adk_runtime_policy
...
def _before_tool_callback(tool, args: dict[str, Any], tool_context):
    ...
    tracer.start_turn(user_text, turn_id=_turn_id(tool_context))
...
def _turn_id(context) -> str:
    return (
        ...
        or getattr(getattr(context, "invocation_context", None), "invocation_id", None)
        or f"{_session_id(context)}:{hash(_extract_user_text(context))}"
    )
